...
class Brands extends React.Component {
    ...
    fetch = (params = {}) => {
        ...
        reqwest({
            url:
                "/api/auth/brand/datatable" +
                `?${params.shop_id ? `&shop_id=${params.shop_id}` : ""}${
                    params.brand_category_id
                        ? `&brand_category_id=${params.brand_category_id}`
                        : ""
                }`,
            method: "post",
            type: "json",
    ...
    fetchBrandCategories = () => {
        const token = localStorage.getItem("jwt_token");
        reqwest({
            url: "/api/auth/brand-category/active",
            method: "post",
            type: "json",
            headers: {
                Authorization: "Bearer " + token,
    ...
    clearFilter = (pagination) => {
        ...
        if (this.state.shop_id || this.state.brand_category_id) {
            this.fetch({ pagination });
        }
    };
    render() {
        ...
        return (
            <PageLayout>
                <Breadcrumb style={{ margin: "16px 0" }}>
                    <Breadcrumb.Item>{t("brands")}</Breadcrumb.Item>
                </Breadcrumb>
                <Card bordered={false}>
                    <div className="row d-flex flex-row justify-content-between">
                        ...
                        <div className="col-md-4 col-sm-12">
                            <Select
                                placeholder={t("select_brand_category")}
                                style={{ width: "100%", marginBottom: "10px" }}
                                onChange={this.onChangeBrandCategory}
                    ...
                    <PageHeader
                        ...
                        title={t("brands")}
                        subTitle={t("create_remove_and_edit_brands")}
                        extra={
                            isAllowed("/brands/add") && [
                                <Link
                                    to="/brands/add"
                                    key="add"
