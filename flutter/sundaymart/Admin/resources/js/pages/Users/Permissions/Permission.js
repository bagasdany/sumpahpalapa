...
class Permission extends React.Component {
                    ...
                    render: (value, row) => {
                        return (
                            <CheckableTag
                                ...
                                checked={value == 1}
                                onChange={(checked) =>
                                    this.handleChange(row.id, name, checked)
                                }
                            >
    ...
    handleChange = async (id, name, checked) => {
        if(IS_DEMO) {
            message.warn("You cannot change in demo mode");
            return;
        }

        if (name == "Superadmin" || name == "Delivery boy") {
            message.error(
                "Cannot change permissions of Superadmin and Delivery boy"
            );
            return false;
        }
        ...
        return (
            <PageLayout>
                <Breadcrumb style={{ margin: "16px 0" }}>
                    <Breadcrumb.Item>{t("permissions")}</Breadcrumb.Item>
                </Breadcrumb>
                <PageHeader
